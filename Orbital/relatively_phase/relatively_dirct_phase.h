...
namespace orbital::relatively_phase {
	...
	public:
		...
		// check if *this is approximately equal to other (the max different of all elements is less than prec)
		bool isApprox(const dirct &other, const value_t &prec = Eigen::NumTraits<value_t>::dummy_precision()) const {
			return ((posture.toRotationMatrix() - other.posture.toRotationMatrix()).array().abs() < prec).all() &&
				((angular_velocity - other.angular_velocity).array().abs() < prec).all() &&
				((angular_acceleration - other.angular_acceleration).array().abs() < prec).all();
		}
		bool isApproxIdentity(const value_t &prec = Eigen::NumTraits<value_t>::dummy_precision()) const { return this->isApprox(relatively_phase::dirct::identity(), prec); }
		...
		// NOTICE: this operator not satisfy the commutative law
