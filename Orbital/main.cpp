...
int main(void) {
    ...
    auto earth_MeanEquator = make_shared<absolute_phase::dirct>(
        earthmoon_MeanOrbit,
        relatively_phase::dirct(make_quaternion(1.35 * (pi / 180.), vec3(0, 0, 1)) * make_quaternion(23.4 * (pi / 180.), vec3(1, 0, 0)))
        );

    auto moon_MeanOrbit = make_shared<absolute_phase::dirct>(
        earth_MeanEquator,
    ...
    auto moon_CenterPoint = make_shared<absolute_phase::point>(
